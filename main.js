document.addEventListener('DOMContentLoaded', function(){
    ...
    function stop(e){
        ...
        if(name1.value.length == ''){
            warnings.innerHTML += `<p>No has escrito ningun nombre</p> <br>`
            entrar = false;
            
        }else if(name1.value.length < 3){
            warnings.innerHTML += `<p>El nombre es muy corto</p> <br>`
        entrar = false;
    }
    
    if(!regexEmail.test(email.value)){
        warnings.innerHTML += `<p>El email no es valido</p> <br>`
        entrar = false;
    }
    if(message.value == ''){
        warnings.innerHTML += `<p>No has escrito ningun mensaje</p> <br>`
        entrar = false;
        
    }
    if(entrar == true){
        warnings.innerHTML += `<p class="contact__passed">Mensaje Enviado</p> <br>`
